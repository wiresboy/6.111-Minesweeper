...
module sound_effect_manager (
	...
	assign sample_trigger = initialized && sample_trigger_count ==0;
	...
	assign sd_address = 1024+data_request_offset;
...
module sound_effect_player#(parameter LOOP = 0)
	...
	logic fifo_reset;
	logic playing = 0;
	fifo_generator_0 fifo(.clk(clk_100mhz), .srst(reset||fifo_reset), .din(data_request_result), .wr_en(data_ready_strobe), .empty(fifo_empty), .dout(fifo_out), .rd_en( sample_trigger&&(!fifo_empty)&&playing ));
